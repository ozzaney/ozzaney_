...
위 수식과 같이 $x_{new}$를 $f_{\theta^*}(x)$에 대입하면 해당 분포에서 y가 확정적(deterministic)으로 결정되는 것이 아니라 $y_{new}$를 샘플링할 수 있다. 
$p(y|f_{\theta^*}(x_{new}))$는 $f_{\theta^*}(x_{new})$를 모수로 가지는 미리 정해진 분포이다.
