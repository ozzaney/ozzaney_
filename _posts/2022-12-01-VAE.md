...
z가 어떤 확률분포 p(z)를 따른다고 할 때 $g_{\theta}(\cdot)$라는 디코더의 네트워크를 통과하고 나면 만들어지는 $g_{\theta}(z)$가 x와 유사하다면  
$g_{\theta}(\cdot)$는 x를 잘 만들어내는 분포라고 볼 수 있다.
...
VI방법이란 우리가 모르지만 추정하고자 하는 $p(z|x)$(true posterior)이 있을 때, 다루기 쉬운 approximation class q(정규분포 등)를 정하고  
이 분포의 정규분포를 결정짓는 파라미터인 $\phi$를 잘 바꿔가면서 최대한 true posterior에 가깝게 만들어 만든  $q_{\phi}(z|x)$를 만드는 방법이다.
...
$\because \int q_{\phi}(z|x)\,dz=1$   
$= \int log(\frac{p(x,z)}{p(z|x)})q_{\phi}(z|x)\,dz  
$\because p(x) = \frac {p(x,z)}{p(z|x)}$  
