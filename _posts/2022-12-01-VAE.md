...
이를 수식으로 나타내면, $\theta^* = argmin[-logp(y|f_{\theta}(x))]$로 볼 수 있다.
이 수식의 의미는 negative log likelihood를 최소화하는 $\theta$는 MLE로 볼 수 있다.

이때 $f_{\theta^*}(x_{new})$는 최적의 $\theta^* $를 모수로 가지는 특정한 분포(정규분포, 베르누이분포 등)라고 할 수 있다.
따라서 이를 이용해서 새로운 값을 샘플링을 할 수 있다.

$$y_{new}~p(y|f_{\theta^*}(x_{new}))$$

즉 $x_{new}$를 $f_{\theta^*}(x)$에 대입하더라도 deterministic하게 값이 결정되는 것이 아니라 해당 분포에서 값을 샘플링을 할 수 있다.

미리 정해진 분포는 가우시안 분포 또는 베르누이 분포로 정한다. 이를 수식으로 풀어보자.
